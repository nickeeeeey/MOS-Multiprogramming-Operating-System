...
int Allocate()
{
    ...
    int address;  // variable to store the generated address.
    int l = 0;
    
    while(1)
    {
        address = rand() % 30;  // Generate a random number between 0 and 29.
        ...
        for(int i = 0; i <= locs; i++)
        {
            if(flags[i] == address)
            {
                l = 1;  // Set a flag to indicate that the address is already in use.
                break;
            }
        }
        ...
        if(l == 0)
        {
            break;  // If the address is not in use, exit the loop.
        }
        else
        {
            l = 0;  // Reset the flag for the next iteration.
        }
    }
    
    locs++;  // Increment the counter for the number of locations used.
    flags[locs] = address;  // Mark the generated address as used.
    ...
    while (fgets(buffer, 41, fp) != NULL)
    {
        if (buffer[0]=='$' && buffer[1]=='A' && buffer[2]=='M' && buffer[3]=='J')
        {
            ...
            while(1)
            {
                ...
                col=0;
                for(int i=0;i<strlen(buffer)-1;i++)  //Iterates over the characters in the line read from the file.
                {
                        if (buffer[i]==' ') break;
                        M[temp1][col]=buffer[i]; //Stores the current character in the memory block at the calculated index.
                        col++;
                        if(col==4)
                        {
                            temp1++;
            ...
            currentLine =ftell(fp);       //It's storing the current position in the file 
        }
...
void Terminate(int msg){
    T=1; //This indicates that the program is terminating.
...
void Write()
{
    FILE *fp=NULL;
    LLC++;
        if(LLC>jpcb.TLL){             // line limit exceeded error
            Terminate(2);
